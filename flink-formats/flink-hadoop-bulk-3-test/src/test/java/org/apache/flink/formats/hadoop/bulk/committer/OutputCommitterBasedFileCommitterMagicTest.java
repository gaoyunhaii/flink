 ...
 *
 */
public class OutputCommitterBasedFileCommitterMagicTest extends AbstractHadoopFileCommitterTest {
	...
	protected Configuration getConfiguration() {
		...
		configuration.set("fs.s3a.committer.name", "magic");
		configuration.set("fs.s3a.committer.staging.unique-filenames", "false");
		configuration.set("fs.s3a.committer.magic.enabled", "true");
		configuration.set("fs.s3a.committer.magic.conflict-mode", "replace");
	...
	protected Path getBasePath() throws IOException {
		...
		Path path = new Path(S3TestCredentials.getTestBucketUriWithScheme("s3a") + "magic-" + UUID.randomUUID());
