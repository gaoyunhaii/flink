...
import org.apache.flink.api.scala.typeutils.{ScalaNothingTypeInfo, UnitTypeInfo}

/**
 * The Unit type extractor.
 */
class ScalaUnitTypeExtractor extends TypeInformationExtractor  {
  /**
   *
   * @return the classes that the extractor could extract the{ @link TypeInformationExtractor} corresponding to.
   */
  override def getClasses: util.List[Class[_]] = util.Arrays.asList(classOf[Unit])
