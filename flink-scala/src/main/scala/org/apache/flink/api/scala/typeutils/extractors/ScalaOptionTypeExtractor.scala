...
class ScalaOptionTypeExtractor extends TypeInformationExtractor{
  /**
   *
   * @return the classes that the extractor could extract the{ @link TypeInformationExtractor} corresponding to.
   */
  override def getClasses: util.List[Class[_]] = util.Arrays.asList(classOf[Option[_]])
  ...
  override def extract(javaType: Type, context: TypeInformationExtractor.Context): Optional[TypeInformation[_]] = {
    ...
    val hierarchy = TypeHierarchyBuilder.buildParameterizedTypeHierarchy(
      javaType,
      TypeHierarchyBuilder.isSameClass(classOf[Option[_]]),
      TypeHierarchyBuilder.isSameClass(classOf[Option[_]]).or(TypeHierarchyBuilder.assignTo(classOf[Option[_]])))

    val parameterizedOptionType = hierarchy.get(hierarchy.size() - 1)
    ...
    Optional.of(new OptionTypeInfo(elementTypeInfo))
  }
}
