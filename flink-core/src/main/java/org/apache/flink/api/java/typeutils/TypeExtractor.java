...
public class TypeExtractor {
	...
	private TypeInformation<?> privateCreateTypeInfo(Type t) {
		return createTypeInfoWithTypeHierarchy(t, null);
	}
	...
	private <IN1, IN2, OUT> TypeInformation<OUT> privateCreateTypeInfo(Class<?> baseClass, Class<?> clazz, int returnParamPos,
		...
		final ArrayList<Type> typeHierarchy = new ArrayList<Type>();
		final Type returnType = getParameterType(baseClass, typeHierarchy, clazz, returnParamPos);
		final Type resolvedReturnType = resolveTypeFromTypeHierachy(returnType, typeHierarchy, false);
		...
		if (resolvedReturnType instanceof TypeVariable<?>) {
		...
		return (TypeInformation<OUT>) createTypeInfoWithTypeHierarchy(resolvedReturnType, typeVariableBindings);
	}

	private final List<Class<?>> classes = new ArrayList<>();

	@SuppressWarnings({ "unchecked", "rawtypes" })
	private <OUT> TypeInformation<OUT> createTypeInfoWithTypeHierarchy(final Type t, final Map<TypeVariable<?>, TypeInformation<?>> typeVariableBindings) {

		try {
			if (isClassType(t)){
				classes.add(typeToClass(t));
			}
			// check if type information can be created using a type factory
			final TypeInformation<OUT> typeFromFactory = createTypeInfoFromFactory(t, typeVariableBindings);
			...
			// check if type is a subclass of tuple
				...
				final TypeInformation<?>[] subTypesInfo = createSubTypesInfo(t, (ParameterizedType) curT, typeVariableBindings, false);
				// type needs to be treated a pojo due to additional fields
				if (subTypesInfo == null) {
					if (t instanceof ParameterizedType) {
						return (TypeInformation<OUT>) analyzePojo(typeToClass(t), (ParameterizedType) t, typeVariableBindings);
					} else {
						return (TypeInformation<OUT>) analyzePojo(typeToClass(t), null, typeVariableBindings);
					}
				}
				// return tuple info
			...
			// e.g. class MyMapper<E> extends MapFunction<String, E>
				...
				final TypeInformation<OUT> typeInfo = typeVariableBindings == null ? null : (TypeInformation<OUT>) typeVariableBindings.get(t);
				if (typeInfo != null) {
					return typeInfo;
				} else {
			...
			// objects with generics are treated as Class first
				...
				return (TypeInformation<OUT>) privateGetForClass(typeToClass(t), (ParameterizedType) t, typeVariableBindings);
			}
	...
	private TypeInformation<?>[] createSubTypesInfo(Type originalType, ParameterizedType definingType,
			final Map<TypeVariable<?>, TypeInformation<?>> typeVariableBindings, boolean lenient) {
		...
		for (int i = 0; i < typeArgumentsLength; i++) {
			...
			if (acutalTypeArgument instanceof TypeVariable<?>) {
				subTypesInfo[i] = typeVariableBindings == null ? null : typeVariableBindings.get(acutalTypeArgument);
				...
				try {
					subTypesInfo[i] = createTypeInfoWithTypeHierarchy(acutalTypeArgument, typeVariableBindings);
				} catch (InvalidTypesException e) {
					if (lenient) {
						subTypesInfo[i] = null;
					} else {
	...
	private <OUT> TypeInformation<OUT> createTypeInfoFromFactory(
			final Type t, final Map<TypeVariable<?>, TypeInformation<?>> typeVariableBindings) {

		final List<Type> factoryHierarchy = new ArrayList<Type>(Arrays.asList(t));
		final TypeInfoFactory<? super OUT> factory = getClosestFactory((ArrayList<Type>) factoryHierarchy, t);
		if (factory == null) {
			return null;
		}
		...
		if (factoryDefiningType instanceof ParameterizedType) {
			...
			final TypeInformation<?>[] subtypeInfo = createSubTypesInfo(t, paramDefiningType, typeVariableBindings, true);
	...
	private <X> TypeInformation<X> privateGetForClass(Class<X> clazz) {
		return privateGetForClass(clazz, null, null);
	}
	...
	private <OUT> TypeInformation<OUT> privateGetForClass(Class<OUT> clazz,
		...
		final TypeInformation<OUT> typeFromFactory = createTypeInfoFromFactory(clazz, typeVariableBindings);
		if (typeFromFactory != null) {
			return typeFromFactory;
		}
		...
		if (countTypeInHierarchy(classes, clazz) > 1) {
			return new GenericTypeInfo<>(clazz);
		}
		...
		try {
			TypeInformation<OUT> pojoType = analyzePojo(clazz, parameterizedType, typeVariableBindings);
			if (pojoType != null) {
				return pojoType;
			}
		} catch (InvalidTypesException e) {
	...
	protected <OUT> TypeInformation<OUT> analyzePojo(Class<OUT> clazz,
		...
		for (Field field : fields) {
			...
			if(!isValidPojoField(field, clazz, (ArrayList<Type>) typeHierarchy)) {
			...
			try {
				...
				TypeInformation<?> ti = createTypeInfoWithTypeHierarchy(resolveFeildType, typeVariableBindings);
	...
	private <X> TypeInformation<X> privateGetForObject(X value) {
		...
		final TypeInformation<X> typeFromFactory = createTypeInfoFromFactory(value.getClass(), null);
		if (typeFromFactory != null) {
			return typeFromFactory;
		}
		...
		if (value instanceof Tuple) {
			...
			if(numFields != countFieldsInClass(value.getClass())) {
				...
				return analyzePojo((Class<X>) value.getClass(), null, null); // we immediately call analyze Pojo here, because
