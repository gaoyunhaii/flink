...
public class PendingCheckpoint implements Checkpoint {
    ...
    public PendingCheckpoint(
        ...
        checkArgument(
                checkpointBrief.getTasksToAck().size() > 0,
                "Checkpoint needs at least one vertex that commits the checkpoint");

        this.jobId = checkNotNull(jobId);
        this.checkpointId = checkpointId;
        this.checkpointTimestamp = checkpointTimestamp;
        this.notYetAcknowledgedTasks = checkNotNull(checkpointBrief.getTasksToAck());
        ...
        this.acknowledgedTasks = new HashSet<>(checkpointBrief.getTasksToAck().size());
    ...
    public TaskAcknowledgeResult acknowledgeTasksFinished(ExecutionAttemptID executionAttemptId) {
        ...
        SubtaskStateStats subtaskStateStats =
                new SubtaskStateStats(
                        vertex.getParallelSubtaskIndex(), ackTimestamp, 0, 0, 0, 0, 0, 0, 0, false);
