...
/**
 * The parser for both {@link NettyMessage.BufferResponse} and messages without data buffer.
 */
public class BufferResponseAndNoDataBufferMessageParser extends NoDataBufferMessageParser {
    ...
    public MessageHeaderParseResult parseMessageHeader(int msgId, ByteBuf messageHeader) throws Exception {
        if (msgId == NettyMessage.BufferResponse.ID) {
            ...
            // If the target channel has been released or the buffer size is 0, the response.buffer will be null.
            if (bufferResponse.buffer == null) {
                return MessageHeaderParseResult.discardDataBuffer(bufferResponse, bufferResponse.getBufferSize());
            } else {
                return MessageHeaderParseResult.receiveDataBuffer(bufferResponse, bufferResponse.getBufferSize(), bufferResponse.buffer);
            }
        } else {
