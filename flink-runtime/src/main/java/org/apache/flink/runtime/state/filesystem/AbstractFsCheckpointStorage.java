...
public abstract class AbstractFsCheckpointStorage implements CheckpointStorage {
	 ...
	 * @param baseDirectory The base directory into which the job checkpoints.
	 */
	protected static Path createFinalSnapshotDirectory(Path baseDirectory, long checkpointId) {
		return new Path(baseDirectory, new Path(FINAL_SNAPSHOT_DIR, String.valueOf(checkpointId)));
	}
