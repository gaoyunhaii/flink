...
if [ $STAGE == "$STAGE_COMPILE" ]; then
	MVN="mvn clean install -Dcheckstyle.skip=true -Denforcer.skip=true -pl flink-runtime -am -nsu -Dflink.convergence.phase=install -Pcheck-convergence -Dflink.forkCount=2 -Dflink.forkCountTestPackage=2 -Dmaven.javadoc.skip=true -B -DskipTests $PROFILE -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn"
	$MVN
	EXIT_CODE=$?
